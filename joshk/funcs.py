...
def to_properties(bibtext_d):
    ...
    properties = {
        "properties": {
            "Name": {"title": [{"text": {"content": bibtext_d["title"]}}]},
            "authors": {
                "multi_select": [
                    {"name": keyword} for keyword in bibtext_d["author"].split(" and ")
                ]
            },
            # ! Should go in body
            # "abstract":{
            ...
            #     ]
            # },
            "doi": {"url": bibtext_d["doi"]},
            "issn": {"number": int(bibtext_d["issn"])},
            "issue": {"number": int(bibtext_d["issue"])},
            "journal": {"rich_text": [{"text": {"content": bibtext_d["journal"]}}]},
            "month": {"number": int(bibtext_d["month"])},
            "year": {"number": int(bibtext_d["year"])},
            "pages": {"rich_text": [{"text": {"content": bibtext_d["pages"]}}]},
            "publisher": {"rich_text": [{"text": {"content": bibtext_d["publisher"]}}]},
            "volume": {"number": int(bibtext_d["volume"])},
            "Tags": {
                "multi_select": [
                    {"name": keyword} for keyword in bibtext_d["keywords"].split(",")
                ]
            },
        }
    }
...
def make_request(properties):
    ...
    title = properties["properties"]["Name"]["title"][0]["text"]["content"]
    if r.status_code == 200:
        click.echo(f"Successfully added {title} to Notion")
    else:
        click.echo(f"Failed to upload to Notion, error message below:")
        click.echo(f"Error code {r.status_code}")
